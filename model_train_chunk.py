    ...
    with torch.inference_mode():
        for batch_data in valid_dl:
            ...
            for t in range(0, sequence_len, chunk_size):
                ...
                logger.info(f"[Validation] Chunk {t//chunk_size + 1}: Frames {t} to {min(t+chunk_size, sequence_len) - 1}")
...
def train():
    ...
    dataset_path = "/scratch/leuven/374/vsc37437/EVE_OP"  # dataset path
    label_excel = "/scratch/leuven/374/vsc37437/EVE_OP"  # label path
    validation_dataset_path = "/scratch/leuven/374/vsc37437/EVE_val"
    '''Tau'''
    ...
    for _ in range(1):
        ...
        wandb.init(
            ...
            config={
                ...
                "val_max_steps_per_folder": 10,    # New setting for validation
        ...
        for epoch in range(config.epochs):
                ...
                try:
                    ...
                    for t in range(0, sequence_len, chunk_size):
                        ...
                        with torch.amp.autocast('cuda'):
                            ...
                            chunk_loss = loss_func(outputs, 'gaze', reference_dict) 
                            chunk_losses.append(chunk_loss)
                            logger.info(f"[Train Epoch {epoch+1} | Step {step}] Chunk {t//chunk_size + 1}: Frames {t} to {min(t+chunk_size, sequence_len) - 1}")

                            
                        
                    # Clear gradients
                    optimizer.zero_grad()
                    train_loss = torch.stack(chunk_losses).mean()
                    # Backward pass
                    ...
                    wandb.log(metrics)
                    logger.info(f"Epoch {epoch + 1}, Step {step}, Loss = {train_loss.item():.4f}, ")
            ...
            if val_loss < best_val_loss:
                ...
                os.makedirs("/scratch/leuven/374/vsc37437/models_a100", exist_ok=True)
                # Save the best model directly with a fixed path
                best_model_path = os.path.join("/scratch/leuven/374/vsc37437/models_a100", "best_model.pt")
                torch.save(model.state_dict(), best_model_path)
